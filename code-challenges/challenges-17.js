...
const recursionPattern = (int1, int2) => {
    ...
    const recursion = (int1, int2) => {

        newArr.push(int1);
        if (int1 < 0) { isReach = true; }
        if (isReach) { return newArr; }
        recursion(int1 - int2, int2);
        newArr.push(int1);
...
const filterLinks = (str) => {
    ...
    let lastIdx = str.lastIndexOf(reg[0]);

    let fStr = str.substring(idx, lastIdx);
    return fStr;
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    ...
    if (reg.test(lowerCaseArr)) {

        Arr = lowerCaseArr.match(reg);
